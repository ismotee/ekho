/**
 * Objektiin liittyvät ajat — same pattern as description “Sisällön ajat” / valmistus aikatiedot.
 */
...
function objectHistoryTimeRowHasContent(d: DateDetailWithTemporalMeta | undefined): boolean {
  return dateDetailHasPersistableContent(d)
}

export function ObjectHistoryTimeList({
  dates,
  onChange,
  idPrefix,
  disabled,
  ...
  const col = useRepeatableCollapsedRows(dates, objectHistoryTimeRowHasContent)
    ...
    <fieldset className="record-form-repeatable-fieldset">
      ...
      {dates.map((row, index) => (
        <CollapsibleRepeatableRow
          ...
          disabled={disabled}
          saveItemNoun={t('recordForm.repeatable.saveItemLabels.objectHistoryTimeEntry')}
          summary={
            [referenceFieldFi(row.association), dateDetailSummaryLine(row)].filter(Boolean).join(' · ') ||
            t('recordForm.history.emptyObjectHistoryTime')
          }
        >
          <ReferenceSelect
            id={`${idPrefix}-role-${index}`}
            label={t('recordForm.history.objectHistoryDateRoleLabel')}
            infoKey="recordForm.info.history.associatedDate.association"
          ...
          <DateDetailInputs
            ...
            flatLayout
            dateLabel={t('recordForm.history.objectHistoryDateEntryLegend', { n: index + 1 })}
