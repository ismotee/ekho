/**
 * Fetches YSO "aine" (http://www.yso.fi/onto/yso/p1433) from Finto:
 ...
 *
 * Run: node scripts/fetch_yso_aine_material_component_groups.mjs
 */
...
const ROOT = 'http://www.yso.fi/onto/yso/p1433'
...
export const MATERIAL_COMPONENT_TYPE_GROUPS = ${JSON.stringify(groups, null, 2)} as const

/** Flat union for legacy matching / search. */
export const MATERIAL_COMPONENT_TYPE_FI = ${JSON.stringify(flat, null, 2)} as const
`

  const outPath = join(__dirname, '../frontend/src/data/ysoMaterialComponentGroups.ts')
