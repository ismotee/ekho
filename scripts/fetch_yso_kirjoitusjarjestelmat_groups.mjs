/**
 * Fetches YSO "kirjoitusjärjestelmät" (http://www.yso.fi/onto/yso/p2627) from Finto:
 ...
 *
 * Run: node scripts/fetch_yso_kirjoitusjarjestelmat_groups.mjs
 */
...
const ROOT = 'http://www.yso.fi/onto/yso/p2627'
...
export const INSCRIPTION_SCRIPT_GROUPS = ${JSON.stringify(groups, null, 2)} as const

/** Flat union for legacy matching / search. */
export const INSCRIPTION_SCRIPT_FI = ${JSON.stringify(flat, null, 2)} as const
`

  const outPath = join(__dirname, '../frontend/src/data/ysoKirjoitusjarjestelmatGroups.ts')
