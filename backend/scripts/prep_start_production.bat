...
if "%PREP_RUNSERVER_HOST%"=="127.0.0.1" (
  echo Waitress on %PREP_RUNSERVER_HOST%:8000 ^(local only^). For LAN: set PREP_RUNSERVER_HOST=0.0.0.0 ^& scripts\prep_start_production.bat
) else (
  echo Waitress on %PREP_RUNSERVER_HOST%:8000 ^(WSGI, suited to long prep sessions^)
)

%PY_CMD% manage.py migrate --noinput
if errorlevel 1 exit /b 1
%PY_CMD% -m waitress --listen=%PREP_RUNSERVER_HOST%:8000 ekho_backend.wsgi:application
